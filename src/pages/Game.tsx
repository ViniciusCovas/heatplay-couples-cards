...
const Game = () => {
  ...
  const handleEvaluationSubmit = async (evaluation: EvaluationData) => {
    ...
    try {
      ...
      // Check if backend is paused for testing
      if (isPaused) {
        logger.info('Backend paused - using local state advancement');
        
        // Create local advancement event
        const localChannel = createLocalGameChannel(room.id);
        if (localChannel) {
          const localEvent: LocalGameEvent = {
            type: 'evaluation_complete',
            data: {
              nextPhase: 'card-display',
              nextPlayer: playerNumber === 1 ? 'player2' : 'player1',
              gameFinished: false,
              evaluationBy: evaluatingPlayerId,
              round: responseData?.round_number || 1,
              timestamp: new Date().toISOString()
            }
          };
          
          localChannel.postMessage(localEvent);
          localChannel.close();
          
          // Simulate game advancement locally
          setTimeout(() => {
            updateGameState({ current_phase: 'card-display' });
          }, 1000);
        }
      } else {
        // Use new secure RPC function for backend evaluation
        const { data: rpcResult, error: rpcError } = await supabase.rpc('submit_response_evaluation', {
          response_id_param: pendingEvaluation.responseId,
          player_id_param: evaluatingPlayerId,
          evaluation_data: {
            evaluation: JSON.stringify(enhancedEvaluation),
        ...
        if (rpcError) {
          logger.error('RPC evaluation error', rpcError);
          throw rpcError;
        }
        
        logger.info('RPC evaluation result', rpcResult);
      }
