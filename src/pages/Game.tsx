...
const Game = () => {
  ...
  const handleEvaluationSubmit = async (evaluation: EvaluationData) => {
    ...
    try {
      ...
      // Immediately broadcast evaluation_submit for partner feedback and speed up processing
      await syncAction('evaluation_submit', {
        evaluation: enhancedEvaluation,
        round_number: responseData.round_number,
        responding_player_id: responseData.player_id,
        evaluation_by: evaluatingPlayerId,
        question: getQuestionTextByCardId(gameState?.current_card || currentCard),
        response: pendingEvaluation.response
      });

      // Kick the queue processor to minimize delay
      try {
        await supabase.functions.invoke('process-game-queue');
      } catch (e) {
        logger.warn('process-game-queue invocation failed', e);
      }

      // Wait a moment for trigger/queue to execute and check for sync events
      setTimeout(async () => {
