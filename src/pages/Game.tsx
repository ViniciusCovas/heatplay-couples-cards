...
const Game = () => {
  ...
  // Centralized card selection during interstitial phase only
  useEffect(() => {
    const selectCardDuringInterstitial = async () => {
      // Only run during interstitial phase and only for the evaluator (who becomes next answerer)
      if (gameState?.current_phase === 'interstitial' && 
        ...
        console.log('🎯 Interstitial phase: selecting AI card');
        ...
        if (availableCards.length > 0) {
          ...
          if (selectedCard && aiResult.reasoning) {
            ...
            updatesForGameState = {
              ...
              used_cards: [...usedCardsFromState, selectedCard]
            };
            
            console.log('✅ AI selection successful during interstitial:', updatesForGameState);
          } else {
            ...
            console.log('🎲 Using random card fallback during interstitial:', updatesForGameState);
          }
          
          // Single atomic update from interstitial to card-display with final card
          await updateGameState(updatesForGameState);
          
          console.log('✅ Interstitial card selection completed and synced');
        } else {
          // No more cards, finish level
          console.log('🏁 No more cards available - finishing level during interstitial');
          
          if (currentLevel >= 4) {
            await updateGameState({ current_phase: 'final-report' });
          } else {
            await updateGameState({ current_phase: 'level-up-confirmation' });
          }
        }
      }
    };
  ...
  }, [gameState?.current_phase, isMyTurn, levelCards, room, isGeneratingCard, currentLevel, i18n.language, gameState?.used_cards]);
  ...
  const handleEvaluationSubmit = async (evaluation: EvaluationData) => {
    ...
    try {
      ...
      console.log('🎯 Moving to interstitial phase for AI card selection');
      await updateGameState({
  ...
  return (
    <div className="min-h-screen bg-background p-4 flex flex-col">
      <div className="w-full max-w-md mx-auto space-y-6 flex-1">
        ...
        {gamePhase === 'interstitial' && <InterstitialScreen />}
