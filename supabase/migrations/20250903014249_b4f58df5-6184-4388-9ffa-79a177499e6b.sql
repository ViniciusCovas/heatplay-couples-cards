-- Fix the handle_evaluation_completion trigger with simplified, more robust logic
  ...
  IF NEW.evaluation IS NOT NULL AND (OLD.evaluation IS NULL OR OLD.evaluation IS DISTINCT FROM NEW.evaluation) THEN
    ...
    BEGIN
      ...
      ELSE
        ...
        IF NOT FOUND THEN
          RAISE EXCEPTION 'Failed to update room state for room %', NEW.room_id;
        END IF;
        ...
        RAISE LOG 'Evaluation completed for room % round %, advanced to card-display with turn: % and card: %', 
