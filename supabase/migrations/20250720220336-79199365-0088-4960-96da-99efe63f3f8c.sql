...
-- Step 1: Clean up duplicate entries from level_selection_votes table
-- Keep only the most recent entry for each room_id + player_id combination
DELETE FROM public.level_selection_votes 
WHERE id NOT IN (
  SELECT DISTINCT ON (room_id, player_id) id
  FROM public.level_selection_votes
  ORDER BY room_id, player_id, created_at DESC
);

-- Step 2: Add the unique constraint safely (now that duplicates are removed)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'unique_player_room_vote'
  ) THEN
...
-- Step 3: Create the atomic level selection function
