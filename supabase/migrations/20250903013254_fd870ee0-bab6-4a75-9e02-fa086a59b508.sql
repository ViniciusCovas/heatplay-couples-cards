-- Fix the handle_evaluation_completion trigger to ensure proper room state updates
...
BEGIN
  ...
  IF NEW.evaluation IS NOT NULL AND (OLD.evaluation IS NULL OR OLD.evaluation IS DISTINCT FROM NEW.evaluation) THEN
    ...
    BEGIN
      ...
      -- Get room details including current card
      SELECT level, selected_language, COALESCE(used_cards, '{}'), current_card 
      INTO room_level, room_language, room_used_cards, current_card_id
      ...
      RAISE LOG 'Evaluation completed for room %, round %, total evaluations: %, evaluating player: %', 
        NEW.room_id, current_round, total_evaluations, evaluating_player_number;
      ...
      ELSE
        ...
        -- ATOMICALLY update room to next round state with proper card selection
        UPDATE public.game_rooms 
        SET 
        ...
        DECLARE
          ...
          updated_card text;
        BEGIN
          SELECT current_phase, current_turn, current_card 
          INTO updated_phase, updated_turn, updated_card
          FROM public.game_rooms 
          WHERE id = NEW.room_id;
          
          RAISE LOG 'Room % updated - phase: %, turn: %, card: %', 
            NEW.room_id, updated_phase, updated_turn, updated_card;
        END;
