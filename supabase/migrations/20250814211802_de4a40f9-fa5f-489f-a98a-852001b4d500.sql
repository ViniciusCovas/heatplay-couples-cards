-- Update handle_proximity_response to advance immediately when ANY player responds
CREATE OR REPLACE FUNCTION public.handle_proximity_response(room_id_param uuid, player_id_param text, is_close_param boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ALWAYS advance to level selection when ANY player responds (changed logic)
  UPDATE public.game_rooms 
  SET 
