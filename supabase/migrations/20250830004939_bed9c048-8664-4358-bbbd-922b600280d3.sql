...
BEGIN
  ...
  IF NEW.evaluation IS NOT NULL AND OLD.evaluation IS NULL THEN
    ...
    BEGIN
      ...
      ELSE
        -- Get next card for the game
        SELECT array_agg(id::text) INTO available_cards
        FROM public.questions q
        JOIN public.game_rooms gr ON gr.level = q.level_id::text::integer
        WHERE gr.id = NEW.room_id
          AND q.language = (SELECT selected_language FROM public.game_rooms WHERE id = NEW.room_id)
        ...
        ELSE
          ...
          SELECT id::text INTO next_card_id
          FROM public.questions q
          JOIN public.game_rooms gr ON gr.level = q.level_id::text::integer
          WHERE gr.id = NEW.room_id
            AND q.language = (SELECT selected_language FROM public.game_rooms WHERE id = NEW.room_id)
...
DECLARE
  ...
  next_card_id text;
BEGIN
  -- Find the room ID for FBAD37
  SELECT id INTO target_room_id FROM public.game_rooms WHERE room_code = 'FBAD37';
  
  IF target_room_id IS NOT NULL THEN
    -- Get a random card for level 1 in Spanish
    SELECT id::text INTO next_card_id
    FROM public.questions
    WHERE level_id::text::integer = 1
      AND language = 'es'
      AND is_active = true
    ORDER BY RANDOM()
    LIMIT 1;
